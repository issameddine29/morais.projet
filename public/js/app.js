...
(function() {
    ...
    function initDevisButtons() {
        ...
        devisButtons.forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                // Si on est sur index ET que #devis existe
    ...
    // ============================================================
    // MINI-BARRE FLOTTANTE CONTACTS (RÉSEAUX SOCIAUX)
    // ============================================================
    
    function initContactFloatingBar() {
        // ✅ Anti-doublon strict
        if (document.querySelector('.mc-contact-float')) {
            console.log('ℹ️  Barre contact déjà présente : injection ignorée');
            return;
        }
        
        const bar = document.createElement('div');
        bar.className = 'mc-contact-float';
        bar.setAttribute('aria-label', 'Contacts réseaux sociaux');
        
        // ⚠️  IMPORTANT : Remplace ces URLs par TES vrais liens sociaux
        const links = [
            { 
                name: 'WhatsApp',  
                icon: 'fab fa-whatsapp', 
                href: 'https://wa.me/32478951269?text=Bonjour%20Morais%20Cleaning,%20je%20souhaite%20un%20devis%20pour%20un%20service%20de%20nettoyage.',
                color: '#25D366'
            },
            { 
                name: 'Facebook',  
                icon: 'fab fa-facebook-f', 
                href: 'https://facebook.com/morais.cleaning',
                color: '#1877f2'
            },
            { 
                name: 'Instagram', 
                icon: 'fab fa-instagram',  
                href: 'https://instagram.com/morais.cleaning',
                color: '#e4405f'
            },
            { 
                name: 'LinkedIn',  
                icon: 'fab fa-linkedin-in',
                href: 'https://linkedin.com/company/morais-cleaning',
                color: '#0077b5'
            },
            { 
                name: 'Snapchat',  
                icon: 'fab fa-snapchat-ghost', 
                href: 'https://snapchat.com/add/morais.cleaning',
                color: '#fffc00'
            }
        ];
        ...
        // ✅ NOUVEAU : Système "stop footer" - la barre remonte en bas de page
        const footerEl = document.getElementById('mc-footer') || document.querySelector('footer');
        const baseTransform = 'translateY(-50%)';
        
        const updatePosition = () => {
            // Ne rien faire si on est en mode mobile/tablette (CSS force top: auto)
            const computedTop = window.getComputedStyle(bar).top;
            if (computedTop === 'auto' || !footerEl) return;
            ...
            // Si le footer entre dans l'écran, calculer le chevauchement
            if (footerRect.top < window.innerHeight) {
                const overlap = (barRect.bottom + 16) - footerRect.top; // 16px de marge
                if (overlap > 0) {
                    extraTranslate = -overlap;
                }
            }
            
            // Appliquer le transform avec l'ajustement
            bar.style.transform = `${baseTransform} translateY(${extraTranslate}px)`;
        };
        
        // Écouter le scroll et resize
        ...
        console.log('✅ Mini-barre Contacts initialisée (milieu desktop + stop footer)');
    }
    ...
    async function initApp() {
        ...
        try {
            ...
            initRealisations();
            initWhatsAppButton();
            initContactFloatingBar(); // Mini-barre contacts réseaux sociaux
